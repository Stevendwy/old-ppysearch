...
//				console.log(e.target.getAttribute("data-name"))
	...
	render() {
		...
		let _cursor = "pointer"
		let _isNormal = _unables.length==2 ? true:false;
		let _items = this.props.titles.map((title, index) => {
			//选中处理
			if(index == _selectorIndex && !_isNormal) {
				_className = "MiddleRightSelectorItem MiddleRightSelectorItemSelected"
				_itemColor = "#0D6FB8"
			} else {
			...
			return (
				<div key={index} className={_className}
					...
					style={{color: _isNormal ? "#333":_itemColor, cursor:_isNormal ? "default" : _cursor}}>
					{title}
